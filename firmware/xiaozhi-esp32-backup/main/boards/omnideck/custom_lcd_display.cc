...
height_(height)
{
    ...
    // 帧缓冲必须放在内部 DMA 内存: 若放 PSRAM，SPI 每次传输都要临时
    // 从内部堆再分配一个私有 DMA 缓冲，内存紧张时失败导致整帧丢失。
...
void CustomLcdDisplay::RLCD_Sendbuffera(uint8_t *Data, int len) {
    // 内存紧张时 SPI DMA 缓冲分配可能瞬时失败；丢帧跳过而不是整机 abort
    // （下一轮 LVGL 刷新会重绘该区域）
    esp_err_t err = esp_lcd_panel_io_tx_color(io_handle, -1, Data, len);
    if (err != ESP_OK && err != ESP_ERR_TIMEOUT) {
        ESP_LOGW(TAG, "lcd spi tx color failed: %s, drop frame", esp_err_to_name(err));
    }
}
