...
void Application::ActivationTask() {
    ...
    // Check for new assets version (从 assets 分区加载全集字体/表情等)
    CheckAssetsVersion();

#ifdef CONFIG_OMNIDECK_STANDALONE
    // OmniDeck 独立模式: 不检查/激活小智云端，不建立 MQTT 长连接。
    // 省电（无保活流量）+ 释放 ~80KB 内部内存 + 开机无云端重试延迟；
    // 代价是小智 AI 语音对话不可用（OmniDeck 功能不受影响）。
    ESP_LOGI(TAG, "OmniDeck standalone: skip xiaozhi cloud activation");
#else
